	...
	public boolean onCommand(CommandSender sender, Command command,String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("Action")) {
			 ...
			 if (args.length == 0) {
				 ...
				 sender.sendMessage("§b/Action cha  §e 查询你的一些基本信息  ");
             ...
             if (args[0].equalsIgnoreCase("cha")) {
				 
				 if (!(d.hasPermission("Action.cha"))) {
				 ...
				 if (d.hasPermission("Action.cha")) {
					 if (args.length != 1) {
						 player.sendMessage("§a[动作助手] §c您输入的命令不正确！正确用法：§b/Action cha  §e 查询你的一些基本信息");
						 
						 return true;
					 }
					 
				     player.sendMessage("§b您的游戏ID是 : §e§l" + player.getName());
				     player.sendMessage("§b您的UUID是 : §e§l" + player.getUniqueId());
				     player.sendMessage("§b您的经验值是 : §e§l" + player.getLevel());
				     player.sendMessage("§b您的生命值是 : §e§l" + player.getHealth());
				     player.sendMessage("§b您的饥饿度是 : §e§l" + player.getFoodLevel());
				     player.sendMessage("§b您的行走速度是 : §e§l" + player.getWalkSpeed());
				     player.sendMessage("§b您的飞行速度是 : §e§l" + player.getFlySpeed());
